...
manage_wifi() {
    ...
    while IFS=: read -r in_use signal security ssid; do
        if [ -z "$ssid" ]; then continue; fi # skip networks without SSID
        ...
        # add connection icon if the network is active
    ...
    local chosen_network=$(echo -e "$formatted_list" | wofi -dmenu -i -selected-row 1 -p "Wi-Fi SSID: ")
    local ssid_index=-1
    ...
    if [ -z "$chosen_network" ]; then
        ...
        # determine action based on the state of the network
        local action
        ...
        case $action in
            "󰸋  Connect")
                ...
                if [[ $(echo "$saved_connections" | grep -Fx "$chosen_id") ]]; then
                    ...
                    local wifi_password=$(wofi -dmenu -p "Password: " -password)
